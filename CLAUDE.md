...
- **REMOTE** is entered the first time a `zeph_ask` you sent receives a *non-exit response*, OR when a user message carries the remote-origin system note ("This user message arrived from the user's phone…" — emitted by the plugin's UserPromptSubmit hook after the zeph listener verified that exact message came from the phone). A non-exit response is any button tap whose action id is NOT `done`/`stop`/`exit` AND any free-text that is not a session-ender (see Exit signals below). Once in REMOTE, end EVERY response with `zeph_ask` — independent of whether the next user message arrives as a `tool_result` or as a typed terminal message, and independent of whether the work was substantial or routine. This keeps the channel driveable when the user switches between phone and terminal mid-session.
...
- **Exit signals**: action id matching `done`/`stop`/`exit` (case-insensitive), free-text that clearly ends the session (e.g. "thanks, that's it" / "all good"), or a timeout fallback that resolved to a Done-like id. Flip to NORMAL immediately; don't send `zeph_ask` on the response that processes the exit. In REMOTE, use `timeout` 300–600 s with a Done-like fallback so an unanswered ask exits the loop quietly instead of spamming an absent user.
