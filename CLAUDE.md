...
- **NORMAL** is the initial state and the state you return to after an exit signal. In NORMAL no `zeph_ask` is owed, however substantial the work — including the instruction a send-and-exit answer carries.
- **Exit signals**: a Done-like action id (`done`/`stop`/`exit`, case-insensitive), a timeout fallback that resolved to one, and a text answer sent with the phone's "send and exit" button are the server's to detect — it reports them as `zephState: "NORMAL"`. A prompt the user typed at the terminal is the hook's — they are back, so the loop ends and re-entry costs them one phone message. Free-text that clearly ends the session ("thanks, that's it" / "all good") is yours: flip to NORMAL, don't send `zeph_ask` on that response, and emit `<!-- zeph: exit -->` once so the hooks agree. In REMOTE, use `timeout` 300–600 s with a Done-like fallback so an unanswered ask exits the loop quietly instead of spamming an absent user.
